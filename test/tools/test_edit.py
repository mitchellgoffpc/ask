...
class TestEditTool(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_basic_replace(self):
        with tempfile.NamedTemporaryFile(mode='w', suffix='.txt') as f:
            ...
            lines = (await self.tool.run(file_path=Path(f.name), old_string="world", new_string="universe", replace_all=False)).split('\n')
    ...
    async def test_replace_all_true(self):
        with tempfile.NamedTemporaryFile(mode='w', suffix='.txt') as f:
            ...
            result = await self.tool.run(file_path=Path(f.name), old_string="test", new_string="example", replace_all=True)
            content = Path(f.name).read_text()
    ...
    async def test_multiple_occurrences_without_replace_all(self):
        with tempfile.NamedTemporaryFile(mode='w', suffix='.txt') as f:
            ...
            with self.assertRaises(ToolError) as cm:
                await self.tool.run(file_path=Path(f.name), old_string="foo", new_string="baz", replace_all=False)
    ...
    async def test_response_snippet(self):
        with tempfile.NamedTemporaryFile(mode='w', suffix='.txt') as f:
            ...
            lines = (await self.tool.run(file_path=Path(f.name), old_string="L\nM\nN", new_string="foo\nbar\nbaz", replace_all=False)).split('\n')
